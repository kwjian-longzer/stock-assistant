# 股票助手技能文件（v5.0 数据库驱动版）

> **本文件是项目的"宪法"，每次自动化任务执行时必须首先读取并严格遵守。**
> v5.0: 在v4.0基础上升级为**7维共振金股引擎 + 热度追踪闭环 + 飞书文档集成 + 三栏首页**架构。
> 数据库扩展至20张表（新增moneyflow），金股发现从5维扩展至7维+2加权+1过滤，
> 板块热度追踪补全DB写入并接入金股加权，网站首页改为"观澜洞见/闲看潮涌/踏浪分金"三栏布局。
...
### 教训10：heat_tracker代码未固化（2026-06-28，v5修复）
- **问题**：heat_tracker.py 实现了完整的热度计算、EMA平滑、生命周期判定，但 `upsert_heat_tracking()` 全项目无人调用，`heat_tracking` DB表始终为空。热度追踪功能未接入金股发现和数据采集流程。
- **根因**：模块开发完成后未进行集成检查，缺少"接口先行、实现缺失"的验证机制
- **修复**：v5新增 `write_heat_to_db()` 补全DB写入，新增15:25定时任务触发热度追踪，金股发现引擎接入板块热度加权和退烧过滤
- **教训**：**每个新模块开发完成后，必须通过集成检查清单**（见下方）

---

## 十四、集成检查清单（v5新增，每个模块必须验证）

每个新模块或功能开发完成后，必须确认以下集成点：

- [ ] **DB写入**：是否有函数调用 `db.upsert_*()` 将数据写入对应表
- [ ] **定时任务**：是否在 Schedule 中创建了触发该模块的任务
- [ ] **下游消费**：是否被 gold_stock_discovery / report_generator 等模块引用
- [ ] **API暴露**：是否在 api_server.py 中有对应的 REST 端点
- [ ] **前端展示**：是否在 app.js 中有渲染函数
- [ ] **端到端测试**：是否验证了从采集到展示的完整链路

> **违反此检查清单的模块视为未完成。** 教训10（heat_tracker未固化）就是违反此清单的典型案例。
